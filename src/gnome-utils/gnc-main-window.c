                               ...
                               gpointer user_data)
{
    ...
    entry->merge_id = egg_menu_manager_add_filename (window->ui_merge, pathname, &error);
                                 ...
                                 const gchar *group_name)
{
    ...
    egg_menu_manager_remove (window->ui_merge, entry->merge_id);
    ...
    if (GTK_IS_MENU_ITEM (proxy))
    {
        ...
        else
        {
            ...
            g_signal_connect (proxy, "select",
                              G_CALLBACK(set_tip), NULL);
            g_signal_connect (proxy, "deselect",
                              G_CALLBACK(unset_tip), NULL);
        }
    }
}
...
gnc_main_window_setup_window (GncMainWindow *window)
{
    ...
    gtk_application_set_app_menu (GTK_APPLICATION (g_application_get_default()),
                                  menu_model);
    menu_model = G_MENU_MODEL(egg_menu_manager_get_menu_by_id(window->ui_merge, "toolbar"));
    ...
    if (!gnc_prefs_is_extra_enabled())
    {
        ...
        action = G_SIMPLE_ACTION(g_action_map_lookup_action(G_ACTION_MAP(priv->action_group),
                                             "ExtensionsAction"));
                                    ...
                                    gpointer       window)
{
    ...
    priv = GNC_MAIN_WINDOW_GET_PRIVATE(window);
    var = g_action_get_state(G_ACTION(action));
